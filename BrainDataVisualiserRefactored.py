# Refactors Done:
# Colour configuration refactor
# Isolated menubar creation
# Programming Structure Notes in Docstrings
# Dataplayers Register Formally
# Isolated dataplayer destruction
# Fixed Y-Range Autofit
    ...
    def drawLayout(self):
        ...
        if self.sensors != None and self.sensors != []:# Only plot if sensor names loaded
            if self.sensor_ids[0]%2 == 0:# First sensor is Red
                self.c.create_text(30,20,text=self.sensors[self.sensor_ids[0]],fill=self.app.getOxyCol(),anchor=tk.NW)
            elif self.sensor_ids[0]%2 == 1:# First sensor is Blue
                self.c.create_text(30,20,text=self.sensors[self.sensor_ids[0]],fill=self.app.getDeOxyCol(),anchor=tk.NW)
